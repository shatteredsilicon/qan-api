...
ln -s $(pwd) ${HOME}/go/src/%{provider_prefix}
...
GOTOOLCHAIN=local go build -o ./revel ./vendor/github.com/revel/cmd/revel
GOTOOLCHAIN=local ./revel --historic-build-mode build %{provider_prefix} release prod
...
cp -rpa ./schema    %{buildroot}%{_datadir}/%{name}/schema
cp -rpa ./conf      %{buildroot}%{_datadir}/%{name}/src/%{provider_prefix}/conf
cp -rpa ./service/query/mini.pl %{buildroot}%{_datadir}/%{name}/src/%{provider_prefix}/service/query/mini.pl

install -d -p %{buildroot}%{_sysconfdir}
cp -rpa ./conf/prod.conf %{buildroot}%{_sysconfdir}/ssm-qan-api.conf
